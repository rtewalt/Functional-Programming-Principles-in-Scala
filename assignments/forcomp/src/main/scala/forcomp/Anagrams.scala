...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(c => c).toList map {
    case (c, occurrences) =>
      (c, occurrences.size)
  } sortBy {
    case (c, count) =>
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    wordOccurrences(s.foldLeft("") { case (accum, word) => accum + word })
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagrams(totalOccs: Occurrences): List[Sentence] = totalOccs match {
      case List() => List(Nil)
      case totalOccs => {
        val subOccurrences: List[Occurrences] = combinations(totalOccs)
        for {
          subOcc <- subOccurrences
          if dictionaryByOccurrences contains (subOcc)
          w <- dictionaryByOccurrences(subOcc)
          rest <- anagrams(subtract(totalOccs, subOcc))
        } yield w :: rest
      }
    }
